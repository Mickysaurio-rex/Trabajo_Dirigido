...
export default function Card_show_reserv({ date, usuario, horarios }) {
            ...
            <section id='section_horarios' className="w-[30%] text-white flex flex-col gap-2 overflow-y-auto h-full ">
                ...
                <div >
                    {horarios.map((horario) => (
                        <p key={`hor_${horario.nombre}`} className='text-[12px] md:text-[16px] lg:text-[20px]'>Horario {horario.nombre}: {horario.inicio} - {horario.fin}</p>
                    ))}
                </div>
            </section>
